...
data Context = -- пустой контекст
               ...
               |Fun Control Context
                --внутри аргумнта
               |Arg Control Context
               -- данный контекст означает, что мы в данный момент считаем один из аргументов
...
--из окружения находит сопоставление именно с той переменной, которая нам нужна
...
-- добовляет окружение операции к каждому ее аргументу
...
--step1 : StateCEK -> StateCEK
--step1 s@(State c cs) = case step (c,cs) of
                --Nothing => s
                --Just (c1,cs1) => (State c1 cs1)

-- Было много попыток написать разные функции, но ничего хорошего не вышло
stepnew : StateCMD () state1 state -> StateCMD () state state2
--stepnew Cek1 = Cek4 -- вот если бы такое сработало было бы идеально, но увы!

--делает переходы step пока это возможно
...
--упаковывает начальное выражение в пустое окружение и контекст
start : Expr -> StateCEK
start term =  State (CS term (Env [])) Mt

--получает из завершающего состояния результирующее выражение
